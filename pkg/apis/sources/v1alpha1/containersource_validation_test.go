...
func TestContainerSource_validatePodSpec(t *testing.T) {
	tests := []struct {
		name string
		ps   corev1.PodSpec
	...
	for _, test := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := validatePodSpec(tt.ps)
			if !cmp.Equal(tt.want.Error(), got.Error()) {
...
func TestContainerSource_validateContainer(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: &apis.FieldError{
				Message: `"SINK" is a reserved environment variable`,
...
func TestContainerSource_validateVolume(t *testing.T) {
	tests := []struct {
		name string
		v    corev1.Volume
		want *apis.FieldError
